...
sys.path.insert(0, os.path.join(ROOT, "churn-prediction"))
sys.path.insert(0, os.path.join(ROOT, "telco-churn"))
...
Streamlit UI
  ...
  · Model Performance   — AUC, precision, recall, F1, SHAP global importance
    ...
    def _load_telco():
        from model.train import build as telco_build
        return telco_build()

    artifact = _load_telco()
    metrics = artifact["metrics"]

    from model.data import FEATURE_NAMES as TC_FEATURES, FEATURE_DISPLAY as TC_DISPLAY, CAMPAIGNS
    ...
    with tab1:
        ...
        from model.train import score_batch
        batch = score_batch(artifact, n=30, seed=st.session_state["telco_seed"])
        df = batch["df"]
...
Streamlit UI  (app.py)
  ...
  · Model performance dashboard — metrics from Dataset 1 test split
    ...
    def _load_churn():
        from model.train import build
        return build()

    artifact = _load_churn()
    df = artifact["df"]

    from model.data import FEATURE_NAMES, FEATURE_DISPLAY, FEATURE_RANGES
